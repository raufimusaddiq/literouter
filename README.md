...
[🚀 Quick Start](#-quick-start) • [💡 Features](#-key-features) • [⚖️ LiteRouter vs 9Router](#-literouter-vs-9router) • [📖 Setup](#-setup-guide)
...
LiteRouter is not a rewrite. It is upstream 9Router with a `MINIMAL_PROFILE`
runtime flag, a small hot-path cache, and the ancillary subsystems turned off.
The routing engine, provider registry, transport matrix, dashboard and SQLite
schema are unchanged — which is why upstream fixes stay cherry-pickable.
...
| Model catalog sync | background refresh on | `MODEL_CATALOG_SYNC=off` |
...
Measured on the same host against the identical mock upstream, so the delta is
router overhead rather than upstream variance. Full method in
[`docs/literouter-baseline/phase-latency-resource.md`](./docs/literouter-baseline/phase-latency-resource.md).

Hot path, n=30, non-streaming:

| Metric | 9Router (prod) | LiteRouter | Change |
| --- | --- | --- | --- |
| Median | 16.53 ms | 13.64 ms | −17.5% |
| p95 | 25.36 ms | 19.40 ms | −23.5% |

Real upstream burst, `kn/deepseek-v4-1-flash`, n=24, 8 concurrent:
...
**Image size is a wash** — ~1.03 GB for both. `.next/standalone` (81 MB),
`next` (201.7 MB) and `sql.js` (23 MB, last-resort SQLite driver) dominate, and
none of them are removed by the minimal profile. See
[`docs/literouter-baseline/phase-bundle-analysis.md`](./docs/literouter-baseline/phase-bundle-analysis.md).
...
> The published npm package is upstream 9Router. LiteRouter is built from this
> repository (see *Run from source* below) or pulled from the staging image.
  ...
  <tr>
    ...
    <td align="center" width="320">
      <a href="https://www.youtube.com/watch?v=hPusYX-5Pmw">
        <img src="https://img.youtube.com/vi/hPusYX-5Pmw/maxresdefault.jpg" alt="Hướng Dẫn Setup OpenClaw + LiteRouter: Tạo Bot Zalo AI Tự Động Từ A-Z" width="300"/>
      </a><br/>
      <b>🇻🇳 Tiếng Việt</b><br/>
      <sub>Hướng Dẫn Setup OpenClaw + LiteRouter: Tạo Bot Zalo AI Tự Động Từ A-Z<br/>by <a href="https://github.com/tuanminhhole">tuanminhhole</a></sub>
    </td>
...
> 🎬 **Made a video about LiteRouter?** Submit a [Pull Request](https://github.com/decolua/9router/pulls) adding your video to this section — we'll merge it!

---

## 🛠️ Supported CLI Tools

LiteRouter works seamlessly with all major AI coding tools:
  ...
  <table>
    <tr>
      <td align="center" width="150">
        ...
        <b>Kiro AI</b><br/>
        <sub>GLM 5 + DeepSeek 3.2 + Qwen3 Coder Next<br/>~50 credits/month free</sub>
      </td>
      <td align="center" width="150">
      ...
      <td align="center" width="150">
        ...
        <b>Vertex AI</b><br/>
        <sub>Gemini 3.1 Pro/Flash + 2.5 Flash<br/>$300 credits free</sub>
      </td>
    </tr>
  </table>
</div>
...
Set `X-LiteRouter-Token-Saver: off` to bypass all token savers for one chat request.
...
Headroom is optional and runs separately. LiteRouter calls Headroom's local `/v1/compress` endpoint, then keeps normal routing, fallback, auth, and usage tracking:

```
Client → LiteRouter → Headroom /v1/compress → LiteRouter → provider
```
...
If Headroom is down or returns an error, LiteRouter fails open and sends the original request.
...
- Your CLI tool sends OpenAI format → LiteRouter translates → Provider receives native format
...
> LiteRouter itself **never charges** you anything. You only pay providers directly (if using paid services).
>
...
### 📊 Understanding LiteRouter Costs & Billing

**LiteRouter Billing Reality:**

✅ **LiteRouter software = FREE forever** (open source, never charges)
...
❌ **LiteRouter never sends invoices** or charges your card
...
- **Cheap providers** (GLM, MiniMax): Pay them directly, LiteRouter just routes
- **FREE providers** (Kiro, OpenCode Free, Vertex AI): Genuinely free within their tier limits, no hidden charges
- **LiteRouter**: Never charges anything, ever
...
Combo: "always-on"
  ...
  5. kr/glm-5                  (free via Kiro, ~50 credits/mo)
...
The dashboard tracks your token usage and displays **estimated costs** as if you were using paid APIs directly. This is **not actual billing** - it's a reference to show how much you're saving by using free models or existing subscriptions through LiteRouter.
...
<details>
<summary><b>💳 Will I be charged by LiteRouter?</b></summary>

**No.** LiteRouter is free, open-source software that runs on your own computer. It never charges you anything.

**You only pay:**

- ✅ **Subscription providers** (Claude Code $20/mo, Codex $20-200/mo) → Pay them directly on their websites
- ✅ **Cheap providers** (GLM, MiniMax) → Pay them directly, LiteRouter just routes your requests
- ❌ **LiteRouter itself** → **Never charges anything, ever**

LiteRouter is a local proxy/router. It doesn't have your credit card, can't send invoices, and has no billing system. It's completely free software.
...
LiteRouter just routes your requests to them - there's no "catch" or future billing from LiteRouter itself. They're truly free services, and LiteRouter makes them easy to use with fallback support.
...
3. **Use subscription providers last:**
   ...
   - LiteRouter helps maximize their value through quota tracking
...
LiteRouter's smart fallback prevents surprise charges:
...
**You're in control:** Set spending limits per provider in dashboard, and LiteRouter respects them.
...
**Pro Tip:** Use Opus for complex tasks, Sonnet for speed. LiteRouter tracks quota per model!
...
### GLM-5.1 / GLM-5 (Daily reset, $0.6/1M)
...
**Use:** `kimi/kimi-k2.5`, `kimi/kimi-k2.5`
...
### Kiro AI (GLM 5 + DeepSeek 3.2 + Qwen3 Coder Next FREE)

```bash
Dashboard → Connect Kiro
→ AWS Builder ID, AWS IAM Identity Center, Google, or GitHub
→ ~50 credits/month free (500 trial credits first 30 days)
...
> **Note:** OpenClaw only works with local LiteRouter. Use `127.0.0.1` instead of `localhost` to avoid IPv6 resolution issues.
...
- Add combo: `cc/claude-opus-5 → glm/glm-5.1 → kr/glm-5`
...
Thanks to all contributors who helped make LiteRouter better!
...
**[OmniRoute](https://github.com/diegosouzapw/OmniRoute)** — A full-featured TypeScript fork of LiteRouter. Adds 36+ providers, 4-tier auto-fallback, multi-modal APIs (images, embeddings, audio, TTS), circuit breaker, semantic cache, LLM evaluations, and a polished dashboard. 368+ unit tests. Available via npm and Docker.
...
- **[RTK](https://github.com/rtk-ai/rtk)** ![Stars](https://img.shields.io/github/stars/rtk-ai/rtk?style=flat&color=yellow) — Rust token-saver. LiteRouter ports its compression pipeline to JS → **−20-40% input tokens** on every request.
- **[Caveman](https://github.com/JuliusBrussee/caveman)** ![Stars](https://img.shields.io/github/stars/JuliusBrussee/caveman?style=flat&color=yellow) by **[@JuliusBrussee](https://github.com/JuliusBrussee)** — viral _"why use many token when few token do trick"_. LiteRouter adapts its prompt → **−65% output tokens**.
- **[Ponytail](https://github.com/DietrichGebert/ponytail)** ![Stars](https://img.shields.io/github/stars/DietrichGebert/ponytail?style=flat&color=yellow) by **[@DietrichGebert](https://github.com/DietrichGebert)** — _"lazy senior dev"_ skill. LiteRouter injects its YAGNI-first ladder → **fewer tokens, less code, shorter diffs**.

Huge thanks to these authors — without their work, LiteRouter's token-saving features wouldn't exist. ⭐ them on GitHub!
