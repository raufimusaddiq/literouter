<div align="center">
  <img src="../images/9router.png?1" alt="Панель управления LiteRouter" width="800"/>

  # LiteRouter - Free AI Router
...
### 📺 Полное руководство по настройке - LiteRouter + Claude Code БЕСПЛАТНО

[![Настройка LiteRouter + Claude Code](https://img.youtube.com/vi/raEyZPg5xE0/maxresdefault.jpg)](https://www.youtube.com/watch?v=raEyZPg5xE0)

**🎬 Полное пошаговое руководство:**
- ✅ Установка и настройка LiteRouter
...
LiteRouter бесшовно работает со всеми основными AI-инструментами для кодинга:
...
- Ваш CLI-инструмент отправляет формат OpenAI → LiteRouter транслирует → Провайдер получает родной формат
...
### 📊 Понимание затрат и оплаты в LiteRouter

**Реальность оплаты LiteRouter:**
...
Панель отслеживает ваше использование токенов и показывает **оценочные затраты**, как если бы вы напрямую использовали платный API. Это **не реальная оплата** — это справка, показывающая, сколько вы экономите, используя бесплатные модели или существующие подписки через LiteRouter.
...
<details>
<summary><b>💳 Взимает ли с меня плату LiteRouter?</b></summary>

**Нет.** LiteRouter — это бесплатное ПО с открытым кодом, работающее на вашем собственном компьютере. Оно никогда ничего с вас не взимает.

**Вы платите только:**
- ✅ **Провайдерам подписки** (Claude Code $20/мес, Codex $20-200/мес) → Платите им напрямую на их сайте
- ✅ **Дешёвым провайдерам** (GLM, MiniMax) → Платите им напрямую, LiteRouter только маршрутизирует ваши запросы
- ❌ **Самому LiteRouter** → **Никогда ничего не взимает, никогда**

LiteRouter — это локальный прокси/роутер. У него нет вашей кредитной карты, он не может выставлять счета и не имеет платёжной системы. Это полностью бесплатное ПО.
...
LiteRouter только маршрутизирует ваши запросы к ним — никаких «ловушек» или будущих платежей. Это действительно бесплатные сервисы, а LiteRouter облегчает их использование с поддержкой резервирования.

**Примечание:** Некоторые провайдеры подписки (Antigravity, GitHub Copilot) могут иметь бесплатные пробные периоды, которые позже становятся платными, но об этом чётко уведомляют сами провайдеры, а не LiteRouter.
...
3. **Используйте провайдеров подписки в последнюю очередь:**
   ...
   - LiteRouter помогает максимизировать их ценность через отслеживание квоты
...
Умный механизм резервирования LiteRouter предотвращает неожиданные расходы:
...
**Вы контролируете:** Установите лимиты расходов на каждого провайдера в панели, и LiteRouter будет их соблюдать.
...
**Профи-совет:** Используйте Opus для сложных задач, Sonnet для скорости. LiteRouter отслеживает квоту для каждой модели!
...
**Вариант 1 — Вручную:** Отредактируйте `~/.openclaw/openclaw.json`:
...
> **Примечание:** OpenClaw работает только с локальным LiteRouter. Используйте `127.0.0.1` вместо `localhost`, чтобы избежать проблем с разрешением имён.
...
- Локальный рантайм LiteRouter включает резервирование SSE→JSON для непотоковых вызовов, когда upstream возвращает `text/event-stream`.
...
Спасибо всем, кто помогает делать LiteRouter лучше!
