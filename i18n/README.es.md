<div align="center">
  <img src="../images/9router.png?1" alt="Panel de control de LiteRouter" width="800"/>

  # LiteRouter - Enrutador de IA GRATUITO y ahorrador de tokens
...
LiteRouter no es una reescritura. Es el 9Router original más un flag de runtime
`MINIMAL_PROFILE`, una pequeña caché de hot-path, y los subsistemas auxiliares apagados.
El motor de routing, el registro de providers, la matriz de transportes, el panel y el
schema de SQLite no cambian — por eso los arreglos de upstream siguen siendo cherry-pickeables.

### Qué cambia realmente

| Área | 9Router / LiteRouter |
| --- | --- |
| Perfil runtime | completo | `MINIMAL_PROFILE=true` |
...
> 🎬 **¿Has hecho un video sobre LiteRouter?** Envía una [Pull Request](https://github.com/decolua/9router/pulls) añadiendo tu video a esta sección — ¡lo fusionaremos!

---

## 🛠️ Herramientas CLI compatibles

LiteRouter funciona a la perfección con todas las principales herramientas de código con IA:
  ...
  <table>
    <tr>
      ...
      <td align="center" width="150">
        ...
        <sub>Sin autenticación • Modelos automáticos<br/>gratuito</sub>
      </td>
      <td align="center" width="150">
...
Headroom es opcional y se ejecuta por separado. LiteRouter llama al endpoint local `/v1/compress` de Headroom y luego mantiene el enrutamiento, la reserva, la autenticación y el seguimiento de uso normales:
...
Si Headroom está caído o devuelve un error, LiteRouter falla de forma segura y envía la solicitud original.
...
Combo: "my-coding-stack"
  1. cc/claude-opus-5        (tu suscripción)
  2. glm/glm-5.1               (respaldo económico, $0.6/1M)
  3. kr/glm-5       (reserva gratuita)
...
- Tu herramienta CLI envía el formato OpenAI → LiteRouter lo traduce → El proveedor recibe el formato nativo
...
> LiteRouter por sí mismo **nunca te cobra** nada. Solo pagas a los proveedores directamente (si usas servicios de pago).
>
> **Ejemplo:** Si tu panel muestra "coste total de $290" mientras usas modelos gratuitos, esto representa
> lo que habrías pagado usando APIs de pago directamente. Tu coste real = **$0** (Kiro es gratis dentro del límite).
>
> ¡Piénsalo como un "rastreador de ahorros" que muestra cuánto estás ahorrando al usar modelos gratuitos o
> enrutar a través de LiteRouter!
...
### 📊 Cómo entender los costes y la facturación de LiteRouter

**Realidad de la facturación de LiteRouter:**

✅ **El software de LiteRouter = GRATUITO para siempre** (código abierto, nunca cobra)
...
✅ **Los proveedores GRATUITOS siguen siendo GRATUITOS** (Kiro, OpenCode Free, Vertex = $0 dentro de los límites gratuitos)
❌ **LiteRouter nunca envía facturas** ni carga tu tarjeta
...
- **Proveedores económicos** (GLM, MiniMax): Págales directamente, LiteRouter solo enruta
- **Proveedores GRATUITOS** (Kiro, OpenCode Free, Vertex): Realmente gratuitos para siempre, sin cargos ocultos
- **LiteRouter**: Nunca cobra nada, jamás
...
Combo: "maximize-claude"
  1. cc/claude-opus-5        (usa la suscripción al máximo)
  2. glm/glm-5.1               (respaldo económico cuando se agota la cuota)
  3. kr/glm-5      (reserva gratuita de emergencia)
...
```
Combo: "free-forever"
  1. kr/glm-5      (Claude 4.5 gratuito e ilimitado)
...
```
Combo: "always-on"
  1. cc/claude-opus-5        (la mejor calidad)
  ...
  5. kr/glm-5      (gratuito e ilimitado)
...
El panel realiza el seguimiento de tu uso de tokens y muestra **costes estimados** como si estuvieras usando APIs de pago directamente. Esto **no es facturación real** — es una referencia para mostrar cuánto estás ahorrando al usar modelos gratuitos o suscripciones existentes a través de LiteRouter.

**Ejemplo:**

- **El panel muestra:** "$290 de coste total"
- **La realidad:** Estás usando Kiro (gratuito)
- **Tu coste real:** **$0.00**
...
<details>
<summary><b>💳 ¿LiteRouter me cobrará algo?</b></summary>

**No.** LiteRouter es un software gratuito y de código abierto que se ejecuta en tu propio ordenador. Nunca te cobra nada.

**Solo pagas:**

- ✅ **Proveedores de suscripción** (Claude Code $20/mes, Codex $20-200/mes) → Págales directamente en sus sitios web
- ✅ **Proveedores económicos** (GLM, MiniMax) → Págales directamente, LiteRouter solo enruta tus solicitudes
- ❌ **El propio LiteRouter** → **Nunca cobra nada, jamás**

LiteRouter es un proxy/enrutador local. No tiene tu tarjeta de crédito, no puede enviar facturas y no tiene sistema de facturación. Es software totalmente gratuito.
...
LiteRouter solo enruta tus solicitudes hacia ellos — no hay "trampa" ni facturación futura. Son servicios realmente gratuitos y LiteRouter los hace fáciles de usar con soporte de reserva.
...
- ❌ **Gemini CLI**: Servicio cerrado el 2026-06-18; la entrada del provider queda marcada como `deprecated` y los modelos `gc/` fallan
...
1. **Empieza con un combo 100% gratuito:**
   ...
   ```
   1. vertex/gemini-3-flash-preview ($300 créditos GCP)
   2. kr/deepseek-3.2 (gratuito vía Kiro)
   3. vertex/gemini-2.5-flash ($300 créditos GCP)
   ```
...
3. **Usa los proveedores de suscripción al final:**
   ...
   - LiteRouter te ayuda a maximizar su valor mediante el seguimiento de cuota
...
La reserva inteligente de LiteRouter evita cargos sorpresa:
...
**Tú tienes el control:** Establece límites de gasto por proveedor en el panel y LiteRouter los respeta.
...
**Consejo profesional:** Usa Opus para tareas complejas y Sonnet para velocidad. ¡LiteRouter realiza el seguimiento de la cuota por modelo!
...
### GLM-5.1 / GLM-5 (restablecimiento diario, $0.6/1M)
...
**Uso:** `glm/glm-5.1`, `glm/glm-5`, `glm/glm-5.1`
...
**Uso:** `kimi/kimi-k2.5`, `kimi/kimi-k2.5`
...
> **Nota:** OpenClaw solo funciona con LiteRouter local. Usa `127.0.0.1` en lugar de `localhost` para evitar problemas de resolución IPv6.
...
- Añade el combo: `cc/claude-opus-5 → glm/glm-5.1 → kr/glm-5`

**Token OAuth caducado**

- LiteRouter lo renueva automáticamente
...
¡Gracias a todos los colaboradores que ayudaron a mejorar LiteRouter!
...
**[OmniRoute](https://github.com/diegosouzapw/OmniRoute)** — Un fork de LiteRouter en TypeScript con todas las funciones. Añade más de 36 proveedores, reserva automática de 4 niveles, APIs multimodales (imágenes, embeddings, audio, TTS), interruptor de circuitos, caché semántica, evaluaciones de LLM y un panel pulido. Más de 368 pruebas unitarias. Disponible a través de npm y Docker.
...
- **[RTK](https://github.com/rtk-ai/rtk)** ![Stars](https://img.shields.io/github/stars/rtk-ai/rtk?style=flat&color=yellow) — ahorrador de tokens en Rust. LiteRouter porta su pipeline de compresión a JS → **−20-40% de tokens de entrada** en cada solicitud.
- **[Caveman](https://github.com/JuliusBrussee/caveman)** ![Stars](https://img.shields.io/github/stars/JuliusBrussee/caveman?style=flat&color=yellow) por **[@JuliusBrussee](https://github.com/JuliusBrussee)** — viral _"why use many token when few token do trick"_. LiteRouter adapta su prompt → **−65% de tokens de salida**.
- **[Ponytail](https://github.com/DietrichGebert/ponytail)** ![Stars](https://img.shields.io/github/stars/DietrichGebert/ponytail?style=flat&color=yellow) por **[@DietrichGebert](https://github.com/DietrichGebert)** — habilidad del _"dev sénior perezoso"_. LiteRouter inyecta su jerarquía primero-YAGNI → **menos tokens, menos código, diffs más cortos**.

Un enorme agradecimiento a estos autores — sin su trabajo, las funciones de ahorro de tokens de LiteRouter no existirían. ¡Dales una ⭐ en GitHub!
