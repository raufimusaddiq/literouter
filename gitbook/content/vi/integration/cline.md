...
Tích hợp LiteRouter với extension Cline VSCode để định tuyến request AI qua hệ thống routing thông minh của LiteRouter.
...
Đặt base URL tới endpoint LiteRouter:

**Cho LiteRouter cục bộ:**
```
http://localhost:20128/v1
```

**Cho LiteRouter cloud:**
```
https://9router.com
```

**Các bước:**
1. Trong field **Base URL**, nhập endpoint LiteRouter
2. Đảm bảo bao gồm `/v1` ở cuối

### 4. Thêm API Key

1. Trong field **API Key**, nhập API key LiteRouter của bạn
2. Bạn có thể tìm API key trong LiteRouter dashboard tại **Settings → API Keys**
3. Key bắt đầu bằng `sk-`

### 5. Chọn Model

1. Trong dropdown **Model**, bạn có thể:
   - Chọn từ model có sẵn (nếu Cline auto-detect)
   - Nhập tên model thủ công từ cấu hình LiteRouter
...
3. Nhấn Enter để gửi
4. Cline sẽ dùng LiteRouter để xử lý request

### Tạo Code

1. Yêu cầu Cline tạo code: "Create a React component for a login form"
2. Cline sẽ tạo code qua LiteRouter
...
3. Nhận giải thích AI qua LiteRouter

### Thao tác File

1. Yêu cầu Cline tạo, sửa hoặc xóa files
2. Cline sẽ dùng LiteRouter để hiểu context và thực hiện thay đổi
...
2. Xác minh LiteRouter instance đang chạy và healthy
3. Thử reload cửa sổ VSCode (Cmd/Ctrl + Shift + P → "Reload Window")
4. Kiểm tra logs LiteRouter để xem lỗi
...
1. Trong Cline settings, đặt Base URL: `https://ai-staging.investdx.biz.id`
2. Đảm bảo bạn đã cấu hình API key trong LiteRouter cloud dashboard
...
## Tích hợp với Tính năng LiteRouter

### Định tuyến Model

LiteRouter tự động định tuyến request đến provider tốt nhất hiện có dựa trên:
...
Nếu một provider thất bại, LiteRouter tự động fallback sang provider khác đã cấu hình trong dashboard.

### Theo dõi Usage

Giám sát usage Cline qua LiteRouter dashboard:
- Tổng request
- Token usage
- Chi phí mỗi model
- Phân bổ provider
