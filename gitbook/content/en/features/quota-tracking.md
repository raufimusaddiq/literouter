...
LiteRouter provides comprehensive quota tracking for all providers:
...
Monthly: ~50 free credits
Reset: Monthly

Dashboard shows:
  This month: 32 / 50 credits (64%)
...
```
Quota type: Capped credit or promotional
Limit: Kiro caps at ~50 credits/month; Vertex runs off a $300 balance; OpenCode Free's list rotates

Dashboard shows:
  Used today: 5M tokens
  Cost: $0 (within the credit)
  Status: ✅ Available
```
...
Cost breakdown:
  ...
  vertex/gemini-3-flash-preview: $0 (free credit)
```
...
⚠️ Cost spike detected
   ...
   Reason: High GLM 5.1 usage (20M tokens)
...
Morning check:
  ...
  ⚠️ GLM 5.1: 2M tokens left (resets 10AM)
...
**Example:**
```
Current: cc/claude-opus → glm/glm-5.1
  80% via Claude (good)
  20% via GLM ($12/month)

Optimized: cc/claude-opus-5 → glm/glm-5.1 → kr/glm-5
  50% via Claude (subscription)
  30% via GLM ($6/month)
  20% via Kiro (free credit)
...
Dashboard → Quota → Reset Schedule:
  ...
  GLM 5.1: Daily 10:00 AM Beijing Time
...
4. Restart LiteRouter if issue persists
