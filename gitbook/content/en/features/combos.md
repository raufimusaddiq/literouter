...
Combos are **custom fallback chains** that you create in the dashboard. Instead of using a single model, you define a sequence of models that LiteRouter tries in order.
...
LiteRouter automatically tries each model in sequence until one succeeds.
...
```
cc/claude-opus → glm/glm-5.1 → kr/glm-5
...
→ Always include a free-credit tier at the end
...
Request → glm/glm-5.1
  ...
  ❌ MiniMax quota out → Use Kiro free credits
```
...
Models:
  1. kr/glm-5
  2. vertex/gemini-3-flash-preview
  3. oc/<model-id>
```
...
Total: $0 while the credits last (Kiro caps at ~50 credits/month, Vertex runs off a $300 balance)
```
...
Name: multi-sub
Models:
  1. cc/claude-opus-5 (Pro subscription)
  2. cx/gpt-5.5 (Codex subscription)
  3. gh/gpt-5.4 (Copilot subscription)
  4. glm/glm-5.1 (Cheap backup)
  5. kr/glm-5 (Free credit emergency)
```
...
Models:
  ...
  2. kr/glm-5 (free credits, use afternoon)
  3. glm/glm-5.1 (daily 10AM reset, use evening)
  4. minimax/MiniMax-M2.7 (5h rolling, use night)
  5. vertex/gemini-3-flash-preview (credit, emergency)
```
...
```
✅ Good:
glm/glm-5.1 → minimax/MiniMax-M2.7 → cc/claude-opus
...
For experimentation:
kr/glm-5 → vertex/gemini-3-flash-preview
```
...
When limit reached, LiteRouter skips paid models and uses free tier only.
