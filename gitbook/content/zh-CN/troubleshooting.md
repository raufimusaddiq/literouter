...
2. **使用组合回退:**
   ```
   仪表盘 → 组合 → 创建回退链
   示例: cc/claude-opus-5 → glm/glm-5.1 → kr/deepseek-3.2
   ```
...
2. **切换到低价层:**
   ```
   使用: glm/glm-5.1 (每 1M tokens $0.6)
        minimax/MiniMax-M2.7 (每 1M tokens $0.20)
   ```
...
3. **使用免费层:**
   ```
   kr/glm-5 (免费)
   vertex/gemini-3.1-pro-preview (免费)
   kr/glm-5 (免费)
   vertex/gemini-3-flash-preview (免费额度)
   ```
