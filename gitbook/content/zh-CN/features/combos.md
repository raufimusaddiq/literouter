...
组合是你在仪表盘中创建的 **自定义回退链**。它不是单一模型,而是定义一组顺序模型,由 LiteRouter 依次尝试。
...
模型:
  ...
  2. glm/glm-5.1 (#1 配额耗尽时)
  3. minimax/MiniMax-M2.7 (#2 配额耗尽时)
```
...
LiteRouter 会按顺序自动尝试每个模型,直到成功为止。
...
请求 → cc/claude-opus-5
  ❌ 配额用完 → cx/gpt-5.5
  ❌ 配额用完 → vertex/gemini-3.1-pro-preview-preview
  ❌ 全部用完 → 返回错误(无低价回退)
```
...
模型:
  ...
  2. vertex/gemini-3-flash-preview (每日 1K, 下午用)
  3. glm/glm-5.1 (每日 10AM 重置, 晚上用)
  4. minimax/MiniMax-M2.7 (5h 滚动, 夜里用)
  5. kr/glm-5 (无限, 应急)
```
...
达到上限时,LiteRouter 跳过付费模型,仅使用免费层。
