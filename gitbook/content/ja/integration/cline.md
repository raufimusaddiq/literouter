...
LiteRouterをCline VSCode拡張機能と統合し、AIリクエストをLiteRouterのインテリジェントルーティングシステム経由でルーティングします。
...
**手順:**
1. **Base URL** フィールドにLiteRouterエンドポイントを入力
2. 末尾に `/v1` を必ず含める
...
2. チャット入力にメッセージを入力
3. Enterを押して送信
4. ClineはLiteRouterを使用してリクエストを処理

### コード生成

1. Clineにコード生成を依頼: 「Create a React component for a login form」
2. ClineはLiteRouterを使用してコードを生成
...
3. LiteRouter経由でAIによる説明を取得

### ファイル操作

1. Clineにファイルの作成、変更、削除を依頼
2. ClineはLiteRouterを使用してコンテキストを理解し変更を加える
3. 受け入れる前に変更を確認
...
2. LiteRouterインスタンスが動作中で正常か確認
3. VSCodeウィンドウをリロードしてみる (Cmd/Ctrl + Shift + P → 「Reload Window」)
4. エラーについてLiteRouterログを確認

## 高度な設定

### クラウドエンドポイントを使用

localhostの代わりにLiteRouterクラウドエンドポイントを使用:

1. Cline設定で、Base URLを設定: `https://ai-staging.investdx.biz.id`
2. LiteRouterクラウドダッシュボードでAPIキーが設定されていることを確認
...
LiteRouterは以下に基づいて最適な利用可能なプロバイダーにリクエストを自動的にルーティング:
- モデル可用性
- プロバイダーヘルスステータス
- コスト最適化
- ロードバランシング
...
プロバイダーが失敗した場合、LiteRouterは自動的にダッシュボードで設定された代替プロバイダーにフォールバックします。

### 使用量トラッキング

LiteRouterダッシュボード経由でCline使用量をモニター:
- 総リクエスト数
- トークン使用量
- モデルごとのコスト
- プロバイダー分布
