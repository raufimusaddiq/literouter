...
LiteRouterをContinue拡張機能と統合し、Visual Studio Codeに直接AIアシスタンスを導入します。

## 前提条件

- Visual Studio Codeがインストール済み
- VSCodeマーケットプレイスからContinue拡張機能がインストール済み
- [ダッシュボード](https://ai-staging.investdx.biz.id/dashboard)からのLiteRouter APIキー
- LiteRouterが動作中 (ローカルまたはクラウド)
...
### モデルが応答しない
- LiteRouterが動作中か確認: `curl http://localhost:20128/health`
