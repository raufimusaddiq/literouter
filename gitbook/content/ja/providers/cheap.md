...
- 💰 **GLM 5.1** - 100万トークンあたり$0.6/$2.2 (日次リセット)
- 💰 **MiniMax M2.7** - 100万トークンあたり$0.2/$1.0 (5時間リセット)
...
|----------|-------------|---------|----------|
| `glm/glm-5.1` | GLM 5.1 | 128K | コーディング、汎用タスク |
...
| モデルID | 説明 | コンテキスト | 最適用途 |
|----------|-------------|---------|----------|
| `minimax/MiniMax-M2.7` | MiniMax M2.7 | 1Mトークン | 長いコンテキスト、コーディング |
...
| モデルID | 説明 | コンテキスト | 最適用途 |
|----------|-------------|---------|----------|
| `kimi/kimi-k2.5` | Kimi Latest | 200K | 汎用コーディング |
...
|----------|----------|-----------|-------|----------|----------|
| **GLM 5.1** | $0.60 | $2.20 | 毎日午前10時 | $6〜22 | 日次クォータユーザー |
| **MiniMax M2.7** | $0.20 | $1.00 | 5時間 | $2〜10 | **最安!** |
...
Models:
  ...
  2. glm/glm-5.1 (低価格バックアップ、日次リセット)
  3. minimax/MiniMax-M2.7 (最安フォールバック)
  4. kr/glm-5 (無料緊急時)
...
```
優先順位:
1. Claude Code(既に支払っているサブスクリプション)
2. GLM 5.1 (低価格バックアップ、100万あたり$0.6)
3. MiniMax M2.7 (最安、100万あたり$0.2)
4. Kiro(無料クレジットの緊急時)

月次コスト例 (1億トークン):
- 3000万は Claude Code 経由: $0 (サブスクリプション)
- 800万は GLM 経由: $4.80
- 200万は MiniMax 経由: $0.40
- 残りは Kiro の無料クレジット: $0
合計: 月$5.20!
```
