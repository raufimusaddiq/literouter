...
リモートアクセスと本番利用のため、VPSまたはDockerにLiteRouterをデプロイ。
...
| `DATA_DIR` | `~/.literouter` | データベースとデータの保存パス |
...
`/etc/nginx/sites-available/literouter` を作成:
...
# リバースプロキシを使用しない場合、LiteRouterポートを許可
