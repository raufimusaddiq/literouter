...
Redis, multi-replica coordination, new OAuth providers, new media support, and
new dashboard redesigns are deferred. They are not PR2 deliverables.

The first implementation target is one production-like process with SQLite and
an in-memory cache. Generic Provider transport configuration follows only after
the existing provider/combo path is extracted and measured.

## 23. Passthrough precedence

Passthrough means no protocol conversion and no unnecessary body reconstruction.
The request may still receive authentication, model routing, safe header
filtering, usage observation, or explicitly enabled token-saving transforms.

Precedence: choose a native transport first; apply only enabled transforms;
translate only when the selected provider cannot serve the incoming transport.
If RTK, Caveman, or Ponytail is disabled, preserve unknown JSON fields wherever
safe. If a transform is enabled, document the fields it changes.

## 24. Usage and quota durability

A request is successful only when the client response succeeds; usage persistence
is an asynchronous bounded side path. Usage events MUST include request ID,
provider, account, model, transport, status, tokens, cost inputs, latency, and
error data available at completion.

The buffer MUST be bounded. On overflow, emit an operational error and apply
the documented drop policy; never block an active stream indefinitely. Shutdown
MUST flush pending events within a fixed timeout. Tests MUST prove that normal
traffic persists complete Usage data.

Quota states are: `available`, `exhausted`, `cooldown`, `unknown`, and `error`.
`available` accounts are preferred; `exhausted` and active `cooldown` accounts
are skipped; `unknown` remains eligible but is lower priority; quota fetch
...
1. Baseline current `main`: capture Usage/Quota/API snapshots, routing fixtures,
   startup time, idle RSS, image size, and representative streaming latency.
2. Extract hot-path state caching without changing provider behavior.
3. Add deterministic tests for native transport, translation fallback, combo,
   round-robin, account fallback, quota states, streaming, and tool calls.
4. Hide or stop initializing one non-retained feature at a time; run the full
...
- all three ingress transports pass native and translated fixtures;
- Kenari and OpenCode Go Chat/Responses paths pass live smoke tests;
