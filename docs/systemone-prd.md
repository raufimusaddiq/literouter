...
- UI: System One menu page with endpoint, setup link, and request example.
...
5. TypeSafe appears in Providers as an API-key provider and its connection test does not perform a billable evaluation.
6. Sidebar exposes System One setup/documentation.
7. Future System One providers require a registry entry with `format: "systemone"`, model list, and endpoint; the route handler needs no provider-specific branch.
