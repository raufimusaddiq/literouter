...
// (contentPrefix). Assertions below read that real surface.
const sysText = (out) =>
...
describe("Claude → Kiro (direct route)", () => {
  ...
  it("does not send additionalModelRequestFields for Kiro models without effort support", () => {
    ...
    expect(sysText(out)).toContain("<max_thinking_length>24576</max_thinking_length>");
  });
  ...
  it("normalizes an unsupported Kiro intensity suffix while preserving agentic behavior", () => {
    ...
    expect(out.additionalModelRequestFields).toBeUndefined();
    expect(sysText(out)).toContain("CHUNKED WRITE PROTOCOL");
  });
  ...
  it("maps Claude-format effort to GPT-5.6 reasoning fields without legacy prompt tags", () => {
    ...
    expect(sysText(out)).not.toContain("<thinking_mode>");
    expect(sysText(out)).not.toContain("<max_thinking_length>");
  });
  ...
  it.each(["auto", "minimal", "ultra"])(
    ...
    (effort) => {
      ...
      expect(out.additionalModelRequestFields).toBeUndefined();
      expect(sysText(out)).toContain("<thinking_mode>enabled</thinking_mode>");
      expect(sysText(out)).toContain("<max_thinking_length>");
    }
  );
  ...
  it("sends Claude system inside the user turn content (no top-level systemPrompt)", () => {
    const out = C2K({
      system: "system-only instruction",
      messages: [{ role: "user", content: "hello" }],
    });

    expect(sysText(out)).toContain("system-only instruction");
